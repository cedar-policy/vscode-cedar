...
suite('Validation RegEx Test Suite', () => {
  ...
  test('validate policy unrecognized entity and action type', async () => {
    ...
    if (result.errors) {
      ...
      assert.equal(result.errors[0].offset, 21);
  ...
  test('validate policy schema Cedar undeclared entity type', async () => {
    const schema = readTestDataFile('undeclared', 'entitytype.cedarschema');
