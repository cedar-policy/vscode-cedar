...
export const PROPERTY_CHAIN_REGEX =
  /\b(?<!\.)(?<element>(([_a-zA-Z][_a-zA-Z0-9]*::)*[_a-zA-Z][_a-zA-Z0-9]*::"(?<id>([^"]*))"|principal|resource|context))(((\.| has )[_a-zA-Z][_a-zA-Z0-9]*|\["([^"]*)"\]))*(?<trigger>.?)$/;
...
export const UNDECLARED_REGEX =
  /`(?<undeclared>.+)` has not been declared as (?<type>(an entity type|a common type|an action))/;
...
export const MISMATCH_ATTR_REGEX =
  /in attribute `(?<attribute>.+)` on `(?<type>.+)::"(?<id>.+)"`, type mismatch: value was expected to have type (\(entity of type )?(?<suggestion>.+)(\))?, but it actually has type (\(entity of type )?(?<unrecognized>.+)(\))?/;
